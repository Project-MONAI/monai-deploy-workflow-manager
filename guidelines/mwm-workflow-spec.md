...
| Property | Type | Description |
|------|------|------|
|id|str|The unique id for this task within the current workflow.|
...
### Allowed Data Attributes for task types
Depending of the type of task object if will contain certain information.

#### Router
These are task types that only allow for task_destinations to be processed.

#### Export
These are task types that allow for artifacts to be exported based on the artifacts Input list.  This task type should not have Out artifacts listed.  
...
| Property | Type | Description |
|------|------|------|
|args|object|An object that will be available to the task plugin when executing this task. This args objects attriubutes are defined by the plugin it's self.|
...
| mandatory| bool | No | Sets the flag if this Artifact is mandatory defaults to false. |
