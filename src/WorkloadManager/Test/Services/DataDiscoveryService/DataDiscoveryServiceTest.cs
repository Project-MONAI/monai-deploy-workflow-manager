...
namespace Monai.Deploy.WorkloadManager.Test.Services.Http
{
    ...
    {
        private readonly Mock<ILogger<DataDiscoveryService>> logger;
        private readonly CancellationTokenSource cancellationTokenSource;

        public DataDiscoveryServiceTest()
        {
            this.logger = new Mock<ILogger<DataDiscoveryService>>();
            this.cancellationTokenSource = new CancellationTokenSource();
        }

        [Fact(DisplayName = "DataDiscoveryService - Constructor Test")]
        public void ConstructorTest()
        {
            Assert.Throws<ArgumentNullException>(() => new DataDiscoveryService(null));
        }

        [Fact(DisplayName = "DataDiscoveryService - Can start and stop service")]
        public async Task CanStartStop()
        {
            var service = new DataDiscoveryService(this.logger.Object);
