...
namespace Monai.Deploy.WorkflowManager.Services
{
    ...
    {
        public static IServiceCollection AddTaskManagerIfEnabled(this IServiceCollection services, HostBuilderContext hostContext)
        {
            Guard.Against.Null(hostContext, nameof(hostContext));
            var config = hostContext.Configuration.GetSection("WorkflowManager:taskManager");

            if (config is null)
            {
                throw new ConfigurationException("WorkflowManager:taskManager is missing");
            }
